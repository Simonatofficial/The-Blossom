...
- **Surface taxonomy (CR-11) — every surface is exactly one of three classes:**
  ...
  - **Popover** (`openPopover(anchor, content)`): quick mid-task utilities — brush color/tool options, overflow (···) menus, quick pickers, sort/filter. Small (≤320px), anchored to its control, no scrim, no route change, dismisses on tap-outside or selection.
