...
- **Surface taxonomy (CR-11) — every surface is exactly one of three classes:**
  - **Page** (`navigate(route)`): content the user works *in* — every widget's internal view (Notes, Journal, Skill, Canvas…), the Canvas focus view. Real navigation: router route, previous content fully unmounts (never visible behind — CR-8), back arrow + browser/hardware back, refresh/deep-links work. Fills the app area over a transparent background so atmosphere + particles stay visible (translucent `surface` containers only).
  - **Panel** (`openPanel(content, opts)`): app/meta surfaces and pickers — settings, theme editor, link picker, module switcher, widget gallery, code dialogs, layer panel. Pop-up overlay from left / right / bottom / full page per the **CR-1 setting** (Settings → Appearance → "Open panels as"; default right ≥600px, bottom sheet below) — the setting governs *panels only*, never pages. Scrim behind; one panel at a time (a second replaces the first).
  - **Popover** (`openPopover(anchor, content)`): quick mid-task utilities — brush color/tool options, overflow (···) menus, quick pickers, sort/filter. Small (≤320px), anchored to its control, no scrim, no route change, dismisses on tap-outside or selection. **Collision-aware (CR-15):** measure, flip to the opposite side of the anchor if the preferred side would cross a viewport edge, clamp to an 8px margin, scroll internally as a last resort — a popover must never extend off-screen.
  - *Decision rule:* go somewhere to work in it → Page; configure or pick something app-level → Panel; adjust mid-task → Popover. Destructive confirmations stay small centered dialogs.
...
- Mixed-shape particles shipped with CR-7: a theme's `particles` is an array of up to 3 layers (each its own emitter, shared canvas/pool/150-cap, per-layer counts scaled proportionally). Flower runs windStreaks + cherryBlossoms; Space runs starfield + comets. Layer reorder in the Effects panel uses up/down chevrons (calmer than drag at 3 items).
