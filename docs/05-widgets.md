...
- **Geometry:** petals are closed Bézier teardrop shapes (two mirrored cubic curves from center to tip), not wedges. Petal length = normalized value (with a 25% minimum so even zero-values look like a bud, never a gap). Petal width ≈ `2π/petalCount` with 8–12% overlap so adjacent petals layer like a real flower. Petals tilt: each rotated to its angle around center with a subtle 3–5° natural variance and drawn back-to-front so overlaps shade correctly.
- **Styling:** each petal gets a radial gradient (deeper hue at base → translucent at tip), 1px lighter rim, and a soft shadow under overlaps. Center: a circular "core" disc (theme `highlight`) with a tiny stamen dot ring. The whole flower sits on a faint stem + two leaves (decorative SVG behind the canvas) when `style: 'botanical'` is on (default for the Blossom module).
