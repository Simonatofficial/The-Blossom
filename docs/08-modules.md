...
## Build decisions - D&D DM Campaign Manager (v1)

- **Shipped as Phase 7.5** — the last big module, reusing existing widgets wherever the spec allowed. New widget types: `statblock`, `loottable`, `sessionlog`, `encounter`, `initiative`, `relationshipweb`. Preset = `js/presets/modules/dnddm.js` (key `dnddm`, icon `dice`), 7 pages: Dashboard · Story · World · NPCs · Encounters · Players · Sessions.
- **NPCs reuse `worldchars`** (the World Builder profile grid — portraits via My Stamps, personality, dated history→Timeline, relationships, secrets, `[[wikilink]]` notes). The **`statblock` widget is a standalone bestiary** (one object per creature) rather than a per-NPC attachment: it keeps `worldchars` untouched and lets Encounters reference one shared bestiary across the whole module via `siblingWidgets`. A creature has a 5e mode (abilities/AC/HP/CR/traits/actions, tap-to-roll using `dnd-shared`'s `rollD20`/`rollFormula`) and a system-agnostic **freeform** mode.
- **World page embeds the World Builder widgets directly** (`worldmap`, `lorewiki`, `civprofile`) rather than cross-linking to a separate World Builder *module* — they are already module-agnostic, so the DM gets a real map + lore + factions in-place with zero new linking code. (Cross-module embedding deferred; note for a future "link an existing world" enhancement.)
- **Encounter difficulty is cozy, not RAW.** `threat = Σ(CR × count)` vs `budget = partySize × partyLevel × 0.25`; the ratio maps to five chips **Sprout · Bud · Bloom · Bright · Radiant** (docs/11 calm-UI: vibes over a math wall). Party size/level live in the encounter widget's settings (default 4 / level 3).
- **One-tap "Run it" pipeline.** An Encounter expands its creatures (× count, named "Goblin 1…N", HP/AC pulled from the referenced stat block) into the sibling `initiative` widget's `config.combatants` — keeping any PCs already added — then emits `dm:init-changed` and navigates there. The InitiativeTracker is the live-play star: turn order (initiative desc), per-combatant HP steppers, condition chips that **count down each round** (0 = indefinite), death-save pips for downed PCs, a round counter, and a giant **Next turn** button for one-handed table use. It refreshes live on `dm:init-changed` while mounted.
- **LootTable** = weighted draws with a Roll button + short per-table history; Encounters can roll a referenced table straight into their treasure notes. **SessionLog** = one object per session (date, attendance chips, prep/recap, loot/XP, and a DM-eyes "what the players don't know yet" field).
- **RelationshipWeb** draws all module `worldchars` (PCs + NPCs) as a force-directed canvas graph; edges come from each character's `char→char` relationships. The sim runs a **bounded heat budget then rests** (no perpetual rAF), pauses while `document.hidden`, re-heats on drag/tap/resize, and caps at 60 nodes — tap a node to open that character.
- **Players page (v1):** roster via `worldchars` (the party as profiles), Inspiration `counter`, an XP/milestones `notes`, and a handouts `gallery`. Importing a D&D Character Manager Blossom code onto this page is **deferred** — the character code already imports through the Codes library; a one-tap "import as PC card" hook is a follow-up.
