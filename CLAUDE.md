...
The Blossom (shipping as **My Blossom**) is a cozy, fully customizable, all-in-one life PWA. The user builds their own app out of **Hubs → Modules → Pages → Tools → Objects**, themed with atmospheres and particles, gamified through the **5 Aspects** (Mental · Physical · Emotional · Social · Recreation) and their growth loop, Skills, Quests, Health, a soul-bonded companion (**Liri**), and a coin Market, grounded in the COSMOS habit method.

> **Tools** is the user-facing name for what the code still calls *widgets* (the rename is user-facing-first — see `docs/17` §7); the word *Widget* is reserved for Android home-screen widgets. A **Hub** is a real layer above Module that packages connected modules and links to other hubs.
